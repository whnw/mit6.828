...
static void boot_map_region_4MB(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm);
...
mem_init(void)
{
	...
	// uint32_t perm = PTE_W | PTE_PS;
	// boot_map_region_4MB(kern_pgdir, KERNBASE, 0xffffffff - KERNBASE + 1, 0, perm);
...
static void
boot_map_region_4MB(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	for (size_t i = 0; i * PTSIZE < size; ++i)
	{
		pgdir[PDX(va + i * PTSIZE)] = (pa + i * PTSIZE) | perm | PTE_P;
	}
}
